...
# 다대일
* 디비 설계상 당연히 다(N)쪽에 외래키가 있어야한다.

# 일대다
* 일이 연관관계에 주인일 경우 (일이 외래키 주인)
* 잘 사용하지 않습니다.

# 일대일
#### * 주 테이블에 외래키
  * 주 객체가 대상 객체의 참조를 가지는것처럼 주테이블에 외래키를 둠
  * 객체지향 개발 , JPA매핑 편의
  * 장점 : 주 테이블만 조회해도 테이블 데이터 확인가능
  * 단점 : 값이 없으면 외래키에 null 허용
#### * 대상 테이블에 외래키
  * 대상 테이블에 외래키 존재
  * 전통적 데이터베이스
  * 장점 : 주 테이블과 대상 테이블 일대일 -> 일대다 관계 변경시에도 테이블 유지
  * 단점 : 프록시 기능 한계로 지연 로딩 설정해도 항상 즉시 로딩이 됨

# 다대다
* 다대다 관계는 RDB로 표현불가능 / 중간 연결테이블 추가하여 다대다 관계를 풀어내야함
* 객체는 컬렉션을 이용해서 가능함 가운데에 LIST LIST
* @ManyToMany / @JoinTable (연결 테이블 생성)
* 실무에서 사용 못함
* 실무에 맞게 : 중간 테입르 @JoinTable -> 엔티티로 올림